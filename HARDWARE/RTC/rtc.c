...
u8 RTC_Init(void)
{
    ...
    if(HAL_RTCEx_BKUPRead(&RTC_Handler, RTC_BKP_DR0) != 0x32F1) //是否第一次配置
    {
        RTC_Set_Time(16, 21, 00, RTC_HOURFORMAT12_PM);	     //设置时间 ,根据实际时间修改
        RTC_Set_Date(19, 9, 25, 3);		                 //设置日期
        HAL_RTCEx_BKUPWrite(&RTC_Handler, RTC_BKP_DR0, 0x32F1); //标记已经初始化过了
    }
